...
class PaymentReceiptGenerator {
  ...
  PaymentReceiptGenerator(this._fileService);
  ...
  pw.Widget _buildPaymentDetailsTable(
    ...
    return pw.Table(
      ...
      children: [
        ...
        _buildTableRow(
          'Payment Date:',
          pw.Text('Date: ${AppDateUtils.formatDate(DateTime.now())}') as String,
        ),
        _buildTableRow('Period Covered:', _getPaymentPeriod(payment)),
        _buildTableRow(
          'Payment Method:',
          _getPaymentMethodDisplay(payment.paymentMethod as PaymentMethod),
        ),
        if (payment.reference != null && payment.reference!.isNotEmpty)
          _buildTableRow('Transaction ID:', payment.reference!),
      ],
    );
  }
  ...
  String _getPaymentPeriod(PaymentModel payment) {
    if (payment.type == PaymentType.rent) {
      final month = AppDateUtils.formatMonth(payment.dueDate);
      return month;
    }
  ...
  Future<String> _saveToFile(pw.Document pdf, String fileName) async {
    ...
    final file = await _fileService.saveDataAsFile(bytes, fileName, 'receipts');
    return file.path;
  }
